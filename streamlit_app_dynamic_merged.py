...
with tabs[2]:
    if n_stocks == 0:
        ...
        st.download_button(
         ...
         # 5. Interactive Normalized Equity Curves Including Portfolio (Plotly)
        ...
        # 6. Advanced Performance Summary Table
        ...
        # 7. Correlation Matrix of Daily Returns
        st.markdown("### Correlation Matrix of Daily Returns")
        daily_returns_df = pd.DataFrame()
        for symbol, eq_curve in all_equity_curves.items():
            daily_returns_df[symbol.upper()] = eq_curve.pct_change()

        corr = daily_returns_df.corr()
        st.write("Correlation between daily returns of stocks:")
        sns.set(style="white")
        fig_corr, ax_corr = plt.subplots(figsize=(9, 7))
        cmap = sns.diverging_palette(220, 10, as_cmap=True)
        sns.heatmap(corr, annot=True, cmap=cmap, center=0, square=True, linewidths=.5, cbar_kws={"shrink": .75}, ax=ax_corr)
        st.pyplot(fig_corr)

        # 8. Download Combined Equity Curves CSV
        combined_eq_df = pd.concat(all_equity_curves.values(), axis=1)
        combined_eq_df.columns = [s.upper() for s in all_equity_curves.keys()]
        combined_eq_df.dropna(how='all', inplace=True)
        csv_data = combined_eq_df.to_csv().encode('utf-8')
